...
from algo.pretrained.transformer import GPT2Model
...
class ObservationEncoder(nn.Module):
    ...
    def forward(
        self, proprio, object_pc, action=None, timesteps=None, attention_mask=None
    ):
        ...
        if self.action_input:
            ...
            stacked_inputs = (
                torch.stack((proprio_embeddings, pc_embeddings), dim=1)
                .permute(0, 2, 1, 3)
        ...
        if self.action_input:
            ...
            x = x.reshape(batch_size, seq_length, 2, self.hidden_size).permute(
                0, 2, 1, 3
            )
