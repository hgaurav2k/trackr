...
class PointNetActorCritic(nn.Module):
    ...
    @torch.no_grad()
    def infer_action_test(self, pos, vel, priv, pc):
        pc_rep = self.point_net(pc).squeeze(1)
        obs = torch.cat([pos,vel,priv,pc_rep],dim=1)
        x = self.actor_mlp(obs)
        mu = self.mu(x)
        return mu
    ...
    def _actor_critic(self, obs_dict):
        ...
        obs = obs_dict['obs']
        pc_info = obs[:,self.pc_begin:self.pc_end].reshape(obs.shape[0],self.pc_num,3)
