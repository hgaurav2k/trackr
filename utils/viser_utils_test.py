...
def draw_mano(server, mano_params):
    ...
    if server.mano is  None:
        server.mano = MANO(
            model_path="/data/jwu/external/manopth/mano/models",            
        ).to('cuda')
    # get faces from mano
...
def add_object(
        ...
        object_viser = ViserUrdf(
                    ...
                    urdf_or_path=yourdfpy.URDF.load(object_urdf_path),
                    root_node_name=f"/world/object_{object_name}",
                )

            
        object_frame = server.server.scene.add_frame(
            f"/world/object_{object_name}",
            position=to_numpy(position),
            wxyz=to_numpy(wxyz) ,
            show_axes=show_axes
        )
        server.handles[object_name] = object_frame
        # return object_frame
        return 


def set_object(server, object_name, position, orientation):
    server.handles[object_name].position = to_numpy(position)
    server.handles[object_name].wxyz = to_numpy(orientation)
    server.handles[object_name].visible = True
    return
