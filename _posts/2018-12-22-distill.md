...
---
  Every day, billions of hours of video capture how humans interact with the world — opening doors, cooking meals, stacking boxes. This data gives away a lot of detail about what we want our robots to do. The guiding question in this project is: *What is the most scalable way to utilize this data for robot learning?*
...
{% include figure.liquid path="assets/intro.png" class="img-fluid rounded z-depth-1" zoomable=true alt="Placeholder figure"%}
  ...
  ~~There is a subtle difference between real2sim and our approach. The former attempts to have a robot imitate the motion of humans in the environment of *the video*. We ask the robot to mimic what a human *would* do if they were acting directly *in the robot's environment*.~~
  ~~This shift makes our approach potentially much more scalable. Indeed, as the human dataset grows, we expect the reward model to capture human motion for a lot of diverse tasks and scenes. The knowledge encoded in this model enables large-scale policy training with reinforcement learning by eliminating the need for per-task manual reward design.~~
  ~~Indeed, as demonstrated in our experiments, a single reward model suffices to train policies across diverse tasks and robot embodiments.~~
...
~~Note, however, that this design decision is by no means necessary. For example, one could train $$\Pi_{h}$$ to predict the 2D motion of fingertips in the image space from RGB observations. We have done some initial steps in this direction (see [here](https://jirl-upenn.github.io/track_reward/)), but training the model requires exponentially more engineering and compute.~~

{% include figure.liquid path="assets/keypoints.png" class="img-fluid rounded z-depth-1" zoomable=true alt="Placeholder figure" caption="*Mapping between human and robot hands* Anthropomorphic hands allow an intuitive mapping of robot links to human hand keypoints. It is defined once and remains consistent across tasks. Here, we show the mapping of the human hand to three different morphologies: an Allegro hand (left), an Xhand (center), and an SVH hand (right)." caption-align="left" %}
...
<div class="row">
    <div class="col-sm" title="Pick up the red crab from the gray tray and place it into the blue bowl">
        {% include video.liquid path="assets/assorted/28.mp4" class="img-fluid rounded z-depth-1" autoplay=true loop=true muted=true %}
    </div>
    <div class="col-sm" title="Pick up the yellow banana from the wooden table with the right hand and place it in the gray bowl">
        {% include video.liquid path="assets/assorted/167.mp4" class="img-fluid rounded z-depth-1" autoplay=true loop=true muted=true %}
    </div>
</div>
<div class="row">
    <div class="col-sm" title="Pick up a grey iPhone from the metal table and place it on the box lid">
        {% include video.liquid path="assets/assorted/1795.mp4" class="img-fluid rounded z-depth-1" autoplay=true loop=true muted=true %}
    </div>
    <div class="col-sm" title="Pick up the blue tongs from the table with the left hand and place them into the tupperware">
        {% include video.liquid path="assets/assorted/13003.mp4" class="img-fluid rounded z-depth-1" autoplay=true loop=true muted=true %}
    </div>
</div>
<figcaption class="figure-caption text-left"><em>Keypoint prediction with RGB input</em> We train a keypoint prediction model on the EgoDex (<em>add cite</em>) dataset conditioned on RGB image, language description of the task and current keypoints. Predicted future hand keypoints are marked in red. Language descriptions can be viewed by hovering the pointer on the corresponding video. Training such a model on more diverse videos and tasks can potentially provide a <em>general reward model</em> that generalizes to new scenes.</figcaption>
</figure>
